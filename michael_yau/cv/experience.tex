...
%---------------------------------------------------------
  \cventry
    {Scala Developer \& Software Developer } % Job title
    {Capgemini (KarrotPay)} % Organization
    {Telford, England} % Location
    {Jan. 2021 - Present} % Date(s)
    {
      \begin{cvitems}
        \item {Tasks, Responsibilities & Experiences}
        \item {Implement frontend designs for HMRC .gov services}
        \item {Continuously improved and helped develope multiple services in various domains, some include Tobacco and Alcohol Customs Duty, Charities Registration, Covid Furlough Scheme, Gov Gateway/SCP etc.}
        \item {Been on various teams rannging from product build teams to live service teams}
        \item {Maintained and improved testing suites both Automated User End2End and Performance Load tests}
        \item {Integrated various inhouse libraries frome encryption libraries, backend NoSql Mongo tool and various others.}
        \item {Investigated and debugged deep rooted issues on mature live service systems}
        \item {Have experience monitoring via Kibana and debugging errors using logs in various environments}
        \item {Studied and reverse engineered live service systems, improving documentation, helping onboarind contractors and new joiners}
        \item {Helped with laptop setups, onboarding and sharing domain and technical knowledge with contractors and new joiners}
      \end{cvitems}
    }
